...
namespace TubeSniper.YouTubeBot.VirtualBrowser
{
	public class VirtualBrowser
	{
		...
		public static VirtualBrowser Create(WebProxy proxy, string cachePath)
		{
			...
			var engine = CreateEngine(proxy, cachePath);
		...
		private static Engine CreateEngine(WebProxy proxy, string cachePath)
		{
			//var currentDirectory = Path.GetDirectoryName(System.Windows.Forms.Application.ExecutablePath);
			//var cachePath = Path.Combine(currentDirectory, "cache", cachePath);
