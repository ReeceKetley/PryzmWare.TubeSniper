...
using TubeSniper.Domain.Models.States;
using TubeSniper.Domain.Models.States.V0;
using TubeSniper.Domain.Models.States.V1;
...
namespace TubeSniper.Domain.Youtube
{
	class YoutubeLoginBotV1
	{
		...
		public YoutubeLoginBotV1(YoutubeBot bot, YoutubeAccount account, ICaptchaService captchaService)
		{
			_bot = bot;
			_account = account;
		...
		public LoginResult Login(YoutubeBot bot, YoutubeAccount account)
		{
			...
			{
				return new LoginResult(LoginResultCode.HttpError, bot.Proxy);
			}
		...
		private void HandleLoginCaptchaState(YoutubeAccount account, LoginCaptchaPageV1 loginCaptchaPage)
		{
			...
			var result = _captchaService.SolveCaptcha(new Uri(img));
			if (result == null)
			{
				throw new Exception();
			}
